...
INT32 Epoll::EpollAdd(baselink* linker)
{
    if(linker == nullptr)
    {
        return -1;
    ...
    m_linkmap[linker->GetFD()] = linker;

    m_ev.data.fd = linker->GetFD();
    return epoll_ctl(m_epfd, EPOLL_CTL_ADD, linker->GetFD(), &m_ev);
...
void Epoll::EpollRemove(INT32 connfd)
{
    ...
    if(m_linktype.find(connfd) != m_linktype.end())
    {
        m_linktype.erase(m_linktype.find(connfd));
    }
}
...
{
    //std::cout << "msg id  = =" << msghead.msgID << std::endl; 
    if(m_linkmap.find(connfd) == m_linkmap.end())
    {
...
void Epoll::SendData(const MesgInfo& msghead,char* body,const INT32 len,const INT32 connfd)
{
    if(m_linkmap.find(connfd) == m_linkmap.end())
    {
        std::cout << "Can't find connfd to send message. connfd = " << connfd << std::endl;
    ...
    INT32 t_hl = t_mh.encode(s_head_buffer, len);

    
    m_linkmap[connfd]->SendData(s_head_buffer, t_hl);
    m_linkmap[connfd]->SendData(body,len);
    return ;
}
