...
bool MySqlMgr::Init() // 1.公有 初始化数据库连接
{
    try {
        ...
        pconn->connect(DATABASE_NAME, DATABASE_IP, DATABASE_USERNAME, DATABASE_PWD);
    }
    catch (BadQuery er)
...
{
    if(strcmp(ColName.c_str(),"rank")!=0 && strcmp(ColName.c_str(), "exp") != 0)
        return false;
...
{
    if(strcmp(ColName.c_str(), "playerName") != 0 && strcmp(ColName.c_str(), "bagInfo") != 0)
        return false;
...
bool MySqlMgr::GetAll(std::map<int,std::pair<int,std::string>>& res)  //8.公有  得到所有玩家的ID
{
    ...
    {
        auto temp = std::make_pair(ares[i]["exp"],ares[i]["playerName"]);
...
std::pair<int,std::string> MySqlMgr::GetPlayerLoginInfo(std::string name)
{
    string a="select id,passwd from login where playerName= '" + name +"'";
    mysqlpp::StoreQueryResult res;
    bool ret =searchFun(a,res);
    if(ret)
    {
        //return std::make_pair(std::stoi(std::string(res[0]["id"])), std::string(res[0]["passwd"]));
        return std::make_pair(res[0]["id"], std::string(res[0]["passwd"]));
    }
