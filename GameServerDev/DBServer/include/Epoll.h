...
public :
    ...
    INT32 EpollAdd(baselink* linker);
    ...
    void RegisterType(INT32 connfd, INT32 type) { m_linktype[connfd] = type; } // 注册连接类型
    INT32 GetType(INT32 connfd) { return m_linktype[connfd]; } // 获取连接类型

private:
    INT32 m_epfd;
    struct epoll_event m_ev;
    struct epoll_event* m_events;
    std::unordered_map<INT32, baselink*> m_linkmap;  // connfd -> baselink
    std::unordered_map<INT32, INT32> m_linktype;  // connfd -> type 
};
#endif
